...
# ─── Nacitani / Trenovani modelu ───────────────────────────────────
def model_path(ticker):
    safe=ticker.replace("=","_").replace("-","_")
    mp=os.path.join(MODELS_DIR, f"{safe}_model.pkl")
    mj=os.path.join(MODELS_DIR, f"{safe}_meta.json")
    return mp, mj
...
def load_or_train(ticker, period="5y"):
    ...
    # ── 1) pokus o nacteni predtrenovaného modelu ──────────────────
    if os.path.exists(mp) and os.path.exists(mj):
        try:
            pkg  = joblib.load(mp)
            meta = json.load(open(mj))
            st.session_state["model_source"]  = "pretrained"
            st.session_state["model_trained"] = meta.get("trained","?")
            st.session_state["auc_val"]  = meta.get("auc_val",0.0)
            st.session_state["auc_tst"]  = meta.get("auc_test",0.0)
            st.session_state["uth_saved"]= meta.get("uth",0.55)
            fi = np.array(pkg.get("fi", np.zeros(len(FCOLS))))
            return pkg["model"], pkg["scaler"], fi, True
        except Exception:
            pass
    # ── 2) fallback – trénuj live ──────────────────────────────────
...
def best_threshold_live(probs, rets):
...
def run_backtest(probs,rets,uth,leverage=1.0,tc=0.0005,margin_rate=0.06,
    ...
    for i in range(n):
        ...
        if sl is not None and in_tr:
            if ne/ent_eq-1<-(sl/leverage): ne=ent_eq*(1.-sl/leverage); p=0.; in_tr=False
...
with h1: st.markdown("## 🧠 "+ticker+" – AI Trader v6")
with h2:
    if st.button("🔄 Refresh",use_container_width=True):
        st.cache_data.clear(); st.cache_resource.clear(); st.rerun()
st.caption("Aktualizace: "+datetime.now().strftime("%d.%m.%Y %H:%M:%S"))

spinner_msg = ("Nacitam predtrenovaný model pro "+ticker+"..."
               if os.path.exists(model_path(ticker)[0])
               else "Trénuji model pro "+ticker+" (muze trvat 2–3 min)...")
...
if model is None or df_daily.empty:
    st.error("Nedostatek dat pro "+ticker+". Zkus jiny symbol.")
    st.stop()

feat, fr = featurize(df_daily)
if len(feat) < 100:
    st.error("Malo dat pro "+ticker+"."); st.stop()
...
if auto_uth:
    uth_ = st.session_state.get("uth_saved",
            best_threshold_live(probs[vas:vae], fr_va)) if is_pretrained \
           else best_threshold_live(probs[vas:vae], fr_va)
else:
    uth_ = man_uth
...
trained_date = st.session_state.get("model_trained","live")
...
st.info(f"Model: {src_tag} | Natrénován: {trained_date[:10] if len(trained_date)>10 else trained_date} | AUC val={auc_v} | AUC test={auc_t}")
...
if PAGE=="Graf":
    ...
    rsi_d=_rsi(cd)
    vol_d=disp.get("Volume",pd.Series(1.,index=disp.index))
    ...
    if st.button("Spustit screener",type="primary"):
        ...
        for idx_s,tk_s in enumerate(tl):
            ...
            try:
                ...
                # Pouzij predtrenovaný model pokud existuje
                mp_,mj_=model_path(tk_s)
                sig_ai="N/A"; prob_ai=0.5
                ...
                rows_s.append({"Ticker":tk_s,
                    ...
                    "RSI":ri,"ADX":ai_,"MA200":"ANO" if abv else "NE",
                    "Rezim":"Bull" if reg else "Bear",
                    "AI Signal":sig_ai,"AI Prob":str(round(prob_ai*100,1))+"%"})
