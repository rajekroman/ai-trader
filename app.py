...
# ============================================================
#  AI Trading Screener v4
#  - Libovolny ticker (akcie, ETF, krypto, komodity)
#  - Paka (1x–10x) s naklady na margin
#  - Fundamentalni data (P/E, EPS, trzni kap., sektor...)
#  - Hloubkova analyza: S/R uroven, multi-timeframe, sektory
#  - VotingClassifier (RF + HistGB) + 20 indikatoru
# ============================================================
...
st.set_page_config(
    page_title="AI Trading Screener v4",
    layout="wide",
    page_icon="📈",
    ...
    try:
        ...
        try:
            ...
            info = {
                ...
                "short_ratio": raw.get("shortRatio", 0),
...
def train_model(ticker, period):
    ...
    df, _ = load_data(ticker, period)
    if df.empty or len(df) < 200:
        return None, None, None, None

    feat, fr = featurize(df)
    N = len(feat); ve = int(N * 0.85)
...
def run_backtest(probs, rets, uth, leverage=1.0, tc=0.0005,
    ...
    daily_margin = margin_rate / 252 * max(leverage - 1, 0)
    equity = [1.0]
    trades, wins = [], []
    ...
    for i in range(n):
        ...
        trade_ret = lev_ret - cost
        new_eq    = equity[-1] * (1.0 + trade_ret)

        if sl is not None and in_trade:
            liq_loss = sl / leverage
            if new_eq / entry_eq - 1 < -liq_loss:
                new_eq   = entry_eq * (1.0 - liq_loss)
                p        = 0.0
                in_trade = False
...
def best_threshold(probs_v, rets_v):
    ...
    for uth in np.arange(0.50, 0.68, 0.01):
        try:
            bt = run_backtest(probs_v, rets_v, float(uth), leverage=1.0,
                              tc=0.0, use_sizing=False)
...
def monthly_pnl(dates, rets):
    ...
    mo_names = {1:"Led",2:"Uno",3:"Bze",4:"Dub",5:"Kve",6:"Cvn",
                7:"Cvc",8:"Srp",9:"Zar",10:"Rib",11:"Lis",12:"Pro"}
    piv.columns = [mo_names.get(m, str(m)) for m in piv.columns]
    return piv.round(2)
...
# ════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("## 📈 AI Trader v4")
    st.markdown("---")

    preset_label = st.selectbox("Vyber instrument", list(PRESETS.keys()),
                                 index=list(PRESETS.keys()).index("Apple (AAPL)"))
    preset_val   = PRESETS.get(preset_label)

    custom = st.text_input("Nebo zadej vlastni ticker (napr. AMD, PLTR, BRK-B)",
                            value="" if preset_val else "")
    ticker = (custom.strip().upper() if custom.strip()
              else (preset_val if preset_val else "AAPL"))

    period = st.selectbox("Historie", ["1y","2y","3y","5y","max"], index=3)
    st.markdown("---")

    st.markdown("#### Paka")
    leverage = st.slider("Paka (leverage)", 1.0, 10.0, 1.0, 0.5)
    margin_rate = st.slider("Rocni naklady na margin (%)", 0.0, 12.0, 6.0, 0.5) / 100
    if leverage > 1:
        st.warning("Paka zvysuje zisky I ztráty! Likvidace pri -" +
                   str(round(100/leverage, 1)) + "% pohybu.")
    st.markdown("---")

    st.markdown("#### Rizeni rizika")
    use_sl      = st.toggle("Stop-Loss", value=True)
    sl_pct      = st.slider("Stop-Loss (%)", 1, 20, 5) / 100 if use_sl else None
    use_sizing  = st.toggle("Confidence position sizing", value=True)
...
#  HLAVNI OBSAH
# ════════════════════════════════════════════════════════════
PAGE = st.radio("", ["Analyza", "Backtest & Paka", "Porovnani s trhem", "Screener"],
                horizontal=True, label_visibility="collapsed")
st.markdown("---")

# ── Nacteni dat ──────────────────────────────────────────────
with st.spinner("Nacitam " + ticker + "..."):
    df, info = load_data(ticker, period)

if df.empty:
    st.error("Ticker " + ticker + " nenalezen nebo neni dostupna data. Zkus jiny symbol.")
    st.stop()

with st.spinner("Trenuju model pro " + ticker + "..."):
    model, sc, feat, fr = train_model(ticker, period)

if model is None:
    st.error("Nedostatek dat pro trenovani modelu (minimum 200 svicek).")
    st.stop()
...
if PAGE == "Analyza":
    ...
    name_str = info.get("name", ticker) if info else ticker
    st.subheader(icon + " " + name_str + " (" + ticker + ")")

    # ── Fundamentalni panel ───────────────────────────────
    if info:
        f1,f2,f3,f4,f5,f6 = st.columns(6)
        mktcap_str = fmt_large(info.get("mktcap", 0))
        pe_str     = str(round(info.get("pe",0),2))     if info.get("pe") else "N/A"
        eps_str    = str(round(info.get("eps",0),2))    if info.get("eps") else "N/A"
        beta_str   = str(round(info.get("beta",0),2))   if info.get("beta") else "N/A"
        div_str    = str(round(info.get("div_yield",0)*100,2))+"%" if info.get("div_yield") else "N/A"
        tgt_str    = str(round(info.get("target_price",0),2)) if info.get("target_price") else "N/A"
        f1.metric("Trzni kap.", mktcap_str)
        f2.metric("P/E ratio",  pe_str)
        f3.metric("EPS",        eps_str)
        f4.metric("Beta",       beta_str)
        f5.metric("Div. yield", div_str)
        f6.metric("Cil. cena",  tgt_str)

        sec_str = info.get("sector","") + " / " + info.get("industry","")
        st.caption("Sektor: " + sec_str)
        st.markdown("---")

    # ── Aktualni signalova karta ──────────────────────────
    ...
    date_now   = feat["date"].iloc[-1].strftime("%d.%m.%Y")
    rsi_now    = round(float(feat["rsi"].iloc[-1]*100), 1)
    adx_now    = round(float(feat["adx"].iloc[-1]*100), 1)
    atr_now    = round(float(feat["atr"].iloc[-1]*100), 2)
    bb_now     = round(float(feat["bb"].iloc[-1]), 2)
    vol_avg    = fmt_large(info.get("avg_volume",0)) if info else "N/A"

    c1,c2,c3,c4,c5 = st.columns(5)
    chg_delta = str(price_chg) + "%"
    c1.metric("Aktualni cena", str(price_now), delta=chg_delta)
    c2.metric("AI Signal", icon + " " + lbl, delta="p=" + str(round(p_now*100,1)) + "%")
    c3.metric("RSI (14)", str(rsi_now),
              delta="prepr." if rsi_now>70 else ("prep." if rsi_now<30 else "neutral"))
    c4.metric("ADX (trend)", str(adx_now),
              delta="silny" if adx_now>25 else "slaby")
    c5.metric("Prah signalu", str(uth_))
    st.markdown("---")

    # ── Svickovy graf + signaly + indikatory ─────────────
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Cena + Signaly", "Volume", "RSI (14)", "AI Pravdepodobnost"),
    )
    ...
    # MA linky
    fig.add_trace(go.Scatter(x=disp["date"],
        y=disp["close"].rolling(20).mean(),
        line=dict(color="#ffd700", width=1), name="MA20"), row=1, col=1)
    fig.add_trace(go.Scatter(x=disp["date"],
        y=disp["close"].rolling(50).mean(),
        line=dict(color="#ff9900", width=1, dash="dot"), name="MA50"), row=1, col=1)
    ...
    for lvl_name, lvl_val in pivots.items():
        ...
        fig.add_hline(y=lvl_val, line_dash="dash", line_color=lc,
                      ...
                      annotation_text=lvl_name, annotation_position="right")
    ...
    if len(long_i):
        fig.add_trace(go.Scatter(x=disp["date"].iloc[long_i],
            y=disp["low"].iloc[long_i] * 0.996,
            mode="markers", marker=dict(symbol="triangle-up", size=9, color="#00c7b7"),
            name="LONG"), row=1, col=1)
    if len(short_i):
        fig.add_trace(go.Scatter(x=disp["date"].iloc[short_i],
            y=disp["high"].iloc[short_i] * 1.004,
            mode="markers", marker=dict(symbol="triangle-down", size=9, color="#ff4b4b"),
            name="SHORT"), row=1, col=1)

    # Volume
    vol_colors = ["#00c7b7" if c >= o else "#ff4b4b"
                  for c, o in zip(disp["close"], disp["open"])]
    fig.add_trace(go.Bar(x=disp["date"], y=disp["volume"],
                         marker_color=vol_colors, name="Volume"), row=2, col=1)

    # RSI
    fig.add_trace(go.Scatter(x=disp["date"], y=disp["rsi"]*100,
    ...
    fig.add_trace(go.Scatter(x=disp["date"], y=d_probs,
                              line=dict(color="#f0c040", width=1.5), name="AI prob"), row=4, col=1)
    fig.add_hline(y=uth_,   line_dash="dash", line_color="green", line_width=0.7, row=4, col=1)
    fig.add_hline(y=1-uth_, line_dash="dash", line_color="red",   line_width=0.7, row=4, col=1)

    fig.update_layout(height=720, showlegend=False,
                       paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                       font_color="#fff", margin=dict(t=30, b=10))
    fig.update_xaxes(rangeslider_visible=False)
    st.plotly_chart(fig, use_container_width=True)

    # Pivot tabulka
    st.subheader("Support & Resistance (Pivot Points – poslednich 20 svicek)")
    piv_df = pd.DataFrame([{"Uroven":k,"Cena":v,"Typ":
                             "Odpor" if "R" in k else ("Podpora" if "S" in k else "Pivot")}
...
# ════════════════════════════════════════════════════════════
    ...
    st.subheader("Backtest s pakou – " + ticker)
    st.info("Paka: " + str(leverage) + "x  |  Stop-Loss: " +
            (str(round(sl_pct*100,1))+"%" if sl_pct else "vypnut") +
            "  |  Margin: " + str(round(margin_rate*100,1)) + "% p.a.")
    ...
    # Porovnej 1x vs paka
    bt1 = run_backtest(te_probs, te_rets, uth_, 1.0,      tc, margin_rate, sl_pct, use_sizing)
    btL = run_backtest(te_probs, te_rets, uth_, leverage,  tc, margin_rate, sl_pct, use_sizing)

    col1, col2 = st.columns(2)
    with col1:
        st.markdown("##### Bez paky (1x)")
        a1,a2,a3 = st.columns(3)
        a1.metric("CAGR",    str(round(bt1["cagr"]*100,1))+"%")
        a2.metric("Sharpe",  str(round(bt1["sharpe"],2)))
        a3.metric("Max DD",  str(round(bt1["max_dd"]*100,1))+"%")
        b1,b2,b3 = st.columns(3)
        b1.metric("Win rate",     str(round(bt1["win_rate"]*100,1))+"%")
        b2.metric("Profit factor",str(round(bt1["profit_factor"],2)))
        b3.metric("Obchodu",      str(bt1["n_trades"]))
    with col2:
        st.markdown("##### S pakou (" + str(leverage) + "x)")
        a1,a2,a3 = st.columns(3)
        a1.metric("CAGR",    str(round(btL["cagr"]*100,1))+"%",
                  delta=str(round((btL["cagr"]-bt1["cagr"])*100,1))+"%")
        a2.metric("Sharpe",  str(round(btL["sharpe"],2)))
        a3.metric("Max DD",  str(round(btL["max_dd"]*100,1))+"%",
                  delta=str(round((btL["max_dd"]-bt1["max_dd"])*100,1))+"%",
                  delta_color="inverse")
        b1,b2,b3 = st.columns(3)
        b1.metric("Win rate",     str(round(btL["win_rate"]*100,1))+"%")
        b2.metric("Profit factor",str(round(btL["profit_factor"],2)))
        b3.metric("Final equity", str(round(btL["final_eq"],3))+"x")

    # Equity curve srovnani
    all_rets = fr.iloc[:N2].fillna(0).values
    all_bt1  = run_backtest(probs, all_rets, uth_, 1.0,     tc, margin_rate, sl_pct, use_sizing)
    all_btL  = run_backtest(probs, all_rets, uth_, leverage, tc, margin_rate, sl_pct, use_sizing)
    ...
    fig_eq.add_trace(go.Scatter(x=dates_eq, y=all_btL["equity"],
                                 mode="lines", name=str(leverage)+"x Paka",
    ...
    fig_eq.update_layout(title="Equity curve – 1x vs " + str(leverage) + "x",
                          height=380, paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                          font_color="#fff", margin=dict(t=45))
    st.plotly_chart(fig_eq, use_container_width=True)

    # Mesicni heatmapa
    st.subheader("Mesicni P&L strategie (%)")
    dates_all = feat["date"].iloc[:len(all_bt1["rets"])].values
    m_pnl = monthly_pnl(dates_all, all_bt1["rets"])
    ...
    fig_hm.update_layout(height=280, paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                          font_color="#fff", margin=dict(t=10), title="")
...
# ════════════════════════════════════════════════════════════
#  PAGE 3 – POROVNANI S TRHEM
# ════════════════════════════════════════════════════════════
elif PAGE == "Porovnani s trhem":
    st.subheader("Relativni sila – " + ticker + " vs SPY / Sektor ETF")

    sector     = info.get("sector","") if info else ""
    sector_etf = SECTOR_ETFS.get(sector, "XLK")

    with st.spinner("Nacitam SPY a " + sector_etf + "..."):
        df_spy, _  = load_data("SPY", period)
        df_sec, _  = load_data(sector_etf, period)

    # Normalizovane ceny
    def norm_price(dff, col="Close"):
        c = dff[col].dropna()
        return c / c.iloc[0]

    if not df_spy.empty and not df_sec.empty:
        spy_norm = norm_price(df_spy).rename("SPY")
        sec_norm = norm_price(df_sec).rename(sector_etf)
        tk_norm  = norm_price(df, "Close").rename(ticker)

        combined = pd.concat([tk_norm, spy_norm, sec_norm], axis=1).ffill().dropna()
        fig_rel = go.Figure()
        colors_r = ["#00c7b7","#888888","#ffd700"]
        for j, col in enumerate(combined.columns):
            fig_rel.add_trace(go.Scatter(x=combined.index, y=combined[col],
                                          mode="lines", name=col,
                                          line=dict(color=colors_r[j], width=2)))
        fig_rel.update_layout(title="Normalizovana cena (start=1)",
                               height=360, paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                               font_color="#fff", margin=dict(t=45))
        st.plotly_chart(fig_rel, use_container_width=True)

        # Relativni sila ticker vs SPY
        rs = combined[ticker] / combined["SPY"]
        fig_rs = go.Figure()
        fig_rs.add_trace(go.Scatter(x=rs.index, y=rs,
                                     mode="lines", name="RS (" + ticker + "/SPY)",
        ...
        # Vykony za ruzna obdobi
        st.subheader("Vykon za obdobi")
        periods_ = {"1M":21,"3M":63,"6M":126,"1Y":252,"2Y":504}
        rows_p   = []
        for pname, pdays in periods_.items():
            row_ = {"Obdobi": pname}
            for sym, dff2 in [(ticker, df),(sector_etf, df_sec),("SPY", df_spy)]:
                if len(dff2) >= pdays:
                    c_ = dff2["Close"].dropna()
                    ret_ = float(c_.iloc[-1] / c_.iloc[-pdays] - 1) * 100
                    row_[sym] = str(round(ret_, 1)) + "%"
                else:
                    row_[sym] = "N/A"
            rows_p.append(row_)
        st.dataframe(pd.DataFrame(rows_p), use_container_width=True, hide_index=True)
    else:
        st.warning("Nepodarilo se nacist porovnavaci data.")
...
# ════════════════════════════════════════════════════════════
    ...
    default_tickers = "AAPL,MSFT,NVDA,TSLA,META,GOOGL,AMZN,SPY,QQQ,GC=F,ETH-USD,BTC-USD"
    tickers_input = st.text_area("Tickery (oddelene carkou)", value=default_tickers, height=80)
    run_screen = st.button("Spustit screener", type="primary")

    if run_screen:
        ticker_list = [t.strip().upper() for t in tickers_input.split(",") if t.strip()]
        rows_s = []
        prog = st.progress(0)
        for idx_s, tk_s in enumerate(ticker_list):
            prog.progress((idx_s+1)/len(ticker_list), text="Analyzuji " + tk_s + "...")
            try:
                df_s, info_s = load_data(tk_s, "1y")
                if df_s.empty or len(df_s) < 100:
                    continue
                feat_s, fr_s = featurize(df_s)
                if len(feat_s) < 60:
                    continue
                ...
                # Rychly signal (bez treninku modelu)
                sig_str = ""
                if rsi_s < 30 and adx_s > 20:
                    sig_str = "🟢 Potencial. LONG"
                elif rsi_s > 70 and adx_s > 20:
                    sig_str = "🔴 Potencial. SHORT"
                elif adx_s < 15:
                    sig_str = "⚪ Sideways"
                else:
                    sig_str = "⚪ Neutral"

                mktcap_s = fmt_large(info_s.get("mktcap",0)) if info_s else "N/A"
                pe_s     = str(round(info_s.get("pe",0),1)) if info_s and info_s.get("pe") else "N/A"
                sector_s = info_s.get("sector","N/A") if info_s else "N/A"
        ...
        if rows_s:
            ...
            csv_screen = pd.DataFrame(rows_s).to_csv(index=False).encode("utf-8")
            st.download_button("Stahnout screener CSV", csv_screen,
                                "screener.csv", "text/csv")
        else:
            st.warning("Zadna data nebyla nactena.")

st.markdown("---")
st.caption("AI Trading Screener v4 | Pouze demonstrace – neni to financni poradenstvi.")
